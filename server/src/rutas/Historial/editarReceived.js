...
try {
  const buscar= await Historial.find({_id:_id});
    ...
    if(buscar[0].received === true){
        const editar= await Historial.findOneAndUpdate({_id:_id},{pending:false,received:false,processing:false,canceled:false})
        res.send(editar)
    } if(buscar[0].received === false){
        const editar= await Historial.findOneAndUpdate({_id:_id},{pending:false,received:true,processing:false,canceled:false})
        res.send(editar)
    }
} catch (error) {
    console.error(error)
